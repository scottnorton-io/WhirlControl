...
class WhirlList:
    ...
    # New CSV helpers
    def to_csv(self) -> str:
        """Serialize members to CSV string."""
        import csv
        import io

        buf = io.StringIO()
        writer = csv.writer(buf)
        writer.writerow(["name", "email", "tags"])
        for m in self.members:
            writer.writerow([m.name, m.email, "|".join(m.tags)])
        return buf.getvalue()

    @classmethod
    def from_csv(cls, data: str) -> "WhirlList":
        """Create a WhirlList from CSV string generated by :meth:`to_csv`."""
        import csv
        import io
        ...
        for row in reader:
            ...
            obj.add_member(row.get("name", ""), row.get("email", ""), tags)
        return obj
